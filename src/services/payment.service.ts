...
import { Prisma, PaymentFrequency, PartyType, DeductionType, PaymentStatus } from '@prisma/client';
...
export class PaymentService {
  ...
  private static calculateFinancials(data: {
    ...
    deductionPercentageTDS?: number | null;
    ptAmountFixed?: number | null;
    deductionPercentageOther?: number | null;
    paidAmount?: number;
  }) {
  ...
  static async getPayments(options: PaymentFilterOptions) {
    ...
    // Apply exact filters
    if (paymentFrequency) whereClause.paymentFrequency = paymentFrequency;
  ...
  static async deletePayment(id: string, hard: boolean = false) {
    const existing = await prisma.payment.findUnique({
      where: { id, deletedAt: null },
    });
