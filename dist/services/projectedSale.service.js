...
exports.ProjectedSaleService = void 0;
const db_1 = require("../db");
class ProjectedSaleService {
    ...
    static async createProjectedSale(data, userId) {
        ...
        return db_1.prisma.$transaction(async (tx) => {
            ...
            if (data.billingType === 'MILESTONE_BASED' && data.milestones && data.milestones.length > 0) {
                const milestonesData = data.milestones.map((ms) => {
    ...
    static async updateProjectedSale(id, data, userId) {
        const existing = await db_1.prisma.projectedSale.findUnique({
    ...
    static async updateStatus(id, status, userId) {
        ...
        if (!existing)
            return null;
        return db_1.prisma.projectedSale.update({
            where: { id },
            data: {
                status,
                updatedBy: userId,
    ...
    static async deleteProjectedSale(id, hard = false) {
        const existing = await db_1.prisma.projectedSale.findUnique({
            where: { id, deletedAt: null },
        });
        if (!existing)
            return null;
        if (hard) {
            await db_1.prisma.projectedSale.delete({
                where: { id },
            });
            return { type: 'HARD' };
        }
        else {
            await db_1.prisma.projectedSale.update({
                where: { id },
                data: {
                    deletedAt: new Date(),
                },
    ...
    static async getProjectedSaleById(id) {
        return db_1.prisma.projectedSale.findUnique({
            where: { id, deletedAt: null },
            include: {
                milestones: {
                    orderBy: { dueDate: 'asc' },
    ...
    static async getProjectedSales(options) {
        const { search, customerId, projectType, billingType, status, startDate, endDate, page = 1, limit = 10, } = options;
        ...
        const [totalCount, projectedSales] = await Promise.all([
            db_1.prisma.projectedSale.count({ where: whereClause }),
            db_1.prisma.projectedSale.findMany({
                where: whereClause,
                include: {
                    milestones: true,
                    customer: {
    ...
    static async addMilestone(projectedSaleId, milestoneData, userId) {
        const sale = await db_1.prisma.projectedSale.findUnique({
    ...
    static async updateMilestone(projectedSaleId, milestoneId, milestoneData, userId) {
        const sale = await db_1.prisma.projectedSale.findUnique({
            where: { id: projectedSaleId, deletedAt: null },
            include: { milestones: true },
        });
    ...
    static async deleteMilestone(projectedSaleId, milestoneId, userId) {
        const sale = await db_1.prisma.projectedSale.findUnique({
            where: { id: projectedSaleId, deletedAt: null },
            include: { milestones: true },
        });
...
exports.ProjectedSaleService = ProjectedSaleService;
